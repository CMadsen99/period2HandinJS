...
describe("## Verify the Friends Facade ##", () => {
  ...
  describe("Verify the editFriend method", () => {
    it("It should change lastName to XXXX", async () => {
        const newLastName = { firstName: "Donald", lastName: "XXXX", email: "dd@b.dk", password: "secret" };
        const status = await facade.editFriend(newLastName.email, newLastName);
        expect(status.modifiedCount).to.equal(1);
        const editedFriend = await friendCollection.findOne({ email: newLastName.email });
        expect(editedFriend.lastName).to.equal(newLastName.lastName);
    })
  })
  ...
  describe("Verify the deleteFriend method", () => {
    ...
    it("It should return false, for a user that does not exist", async () => {
        const deleted = await facade.deleteFriend("XXXX@X.XX");
  ...
  describe("Verify the getVerifiedUser method", () => {
    it("It should correctly validate Peter Admins's credential,s", async () => {
    ...
    it("It should NOT validate Peter Admin's credential,s", async () => {
        const nonVeriefiedPeter = await facade.getVerifiedUser("peter@admin.dk", "hello")
        expect(nonVeriefiedPeter).to.be.null;
    })

    it("It should NOT validate a non-existing users credentials", async () => {
        const nonVeriefiedUser = await facade.getVerifiedUser("xxx@x.xx", "hello")
        expect(nonVeriefiedUser).to.be.null;
    })
  })
